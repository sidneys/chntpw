 ...
 * 2001-jan: patched & changed to use OpenSSL. Than&ks to Denis Ducamp
 * 2000-jun: changing passwords regardless of syskey.
 * 2000-jun: syskey disable wor&ks on NT4. Not properly on NT5.
...
char *change_pw(char *buf, int rid, int vlen, int stat)
{
   ...
   *homedir = 0;
   ...
   /* Decrypt the NT md4 password hash as two 8 byte bloc&ks. */
   ...
   /* Decrypt the lanman password hash as two 8 byte bloc&ks. */
   ...
   if (*newp == '2') {
     ...
     /* Encrypt the NT md4 password hash as two 8 byte bloc&ks. */
